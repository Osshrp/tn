  ...
  class Main
    ...
    def run
      ...
      loop do
        ...
        send(main_menu[input]) if main_menu.key?(input)
      end
    end
    ...
    def create_train
      ...
      puts '2: грузовой'
      input = gets.chomp.to_i
      new_train(train_type[input], train_number) if train_type.key?(input)
    rescue StandardError => e
      puts e.message.to_s
      create_train
    end
    ...
    def attach_wagon
      train = select_train(pick_train_number)[0]
      if train.type == :cargo
        puts 'Введите объем вагона(60 - 256 м3):'
        wagon = RailWay::CargoWagon.new(gets.chomp.to_f)
      else
        puts 'Введите вместимость вагона(40 - 80 мест):'
        wagon = RailWay::PassengerWagon.new(gets.chomp.to_i)
      end
      train.attach_wagon(wagon)
    end

    def detach_wagon
      select_train(pick_train_number)[0].detach_wagon
    end

    def train_to_station
      train_number = pick_train_number
      station_name = pick_station_name
      select_station(station_name)[0].arrive(select_train(train_number)[0])
    end

    def trains_on_station
      select_station(pick_station_name)[0].each_train do |train|
    ...
    def wagons_list
      select_train(pick_train_number)[0].each_wagon do |wagon|
        i = 1
        if wagon.type == :cargo
          puts "№#{i}, Тип: #{wagon.type}, свободный объем: #{wagon.free_volume}," \
           "занятый объем: #{wagon.occupied_volume}"
        else
          puts "№#{i}, Тип: #{wagon.type}, свободные места: #{wagon.free_seats}," \
